 ...
 */
public class XuguServerSessionManager implements DBAServerSessionManager<XuguServerSession>, DBAServerSessionDetailsProvider {
    ...
    public XuguServerSessionManager(DBCExecutionContext executionContext)
    {
    ...
    @Override
    public Collection<XuguServerSession> getSessions(DBCSession session, Map<String, Object> options) throws DBException
    {
        try {
            ...
            try (JDBCPreparedStatement dbStat = ((JDBCSession) session).prepareStatement(sql.toString())) {
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {
                    List<XuguServerSession> sessions = new ArrayList<>();
                    while (dbResult.next()) {
                        sessions.add(new XuguServerSession(dbResult));
                    }
                    return sessions;
                }
            }
    ...
    @Override
    public void alterSession(DBCSession session, XuguServerSession sessionType, Map<String, Object> options) throws DBException
    {
    ...
    public List<DBAServerSessionDetails> getSessionDetails() {
        ...
        extDetails.add(new AbstractServerSessionDetails("Long Operations", "Displays the status of various operations that run for longer than 6 seconds (in absolute time)", DBIcon.TYPE_DATETIME) {
            @Override
            public List<XuguServerLongOp> getSessionDetails(DBCSession session, DBAServerSession serverSession) throws DBException {
                try {
                    try (JDBCPreparedStatement dbStat = ((JDBCSession) session).prepareStatement(
                        "SELECT * FROM V$SESSION_LONGOPS WHERE SID=?"))
                    {
                        dbStat.setLong(1, ((XuguServerSession) serverSession).getSid());
                        try (JDBCResultSet dbResult = dbStat.executeQuery()) {
                            List<XuguServerLongOp> longOps = new ArrayList<>();
                            while (dbResult.next()) {
                                longOps.add(new XuguServerLongOp(dbResult));
                            }
                            return longOps;
                        }
                    }
