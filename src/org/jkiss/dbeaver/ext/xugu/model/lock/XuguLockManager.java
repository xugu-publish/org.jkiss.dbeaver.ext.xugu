 ...
 */
package org.jkiss.dbeaver.ext.xugu.model.lock;
...
import org.jkiss.dbeaver.ext.xugu.editors.XuguLockEditor;
...
public class XuguLockManager extends LockGraphManager implements DBAServerLockManager<XuguLock, XuguLockItem> {
    ...
    public XuguLockManager(XuguDataSource dataSource) {
    ...
    @Override
    public Map<Object, XuguLock> getLocks(DBCSession session, Map<String, Object> options) throws DBException {
        try {

            Map<Object, XuguLock> locks = new HashMap<>(10);

            try (JDBCPreparedStatement dbStat = ((JDBCSession) session).prepareStatement(LOCK_QUERY)) {
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {

                    while (dbResult.next()) {
                        XuguLock l = new XuguLock(dbResult);
                        locks.put(l.getId(), l);
                    }
                }
    ...
    @Override
    public void alterSession(DBCSession session, XuguLock lock, Map<String, Object> options) throws DBException {
        try {
    ...
    @Override
    public Class<XuguLock> getLocksType() {
        return XuguLock.class;
    }


    @Override
    public Collection<XuguLockItem> getLockItems(DBCSession session, Map<String, Object> options)
        throws DBException {
        try {

            List<XuguLockItem> locks = new ArrayList<>();
            ...
            try (JDBCPreparedStatement dbStat = ((JDBCSession) session).prepareStatement(LOCK_ITEM_QUERY)) {
                ...
                switch (otype) {
                    ...
                    case LockManagerViewer.typeWait:
                        dbStat.setInt(1, (int) options.get(XuguLockEditor.sidWait));
                        break;

                    case LockManagerViewer.typeHold:
                        dbStat.setInt(1, (int) options.get(XuguLockEditor.sidHold));
                        break;
                ...
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {
                    ...
                    while (dbResult.next()) {
                        locks.add(new XuguLockItem(dbResult));
                    }
                }
            }
