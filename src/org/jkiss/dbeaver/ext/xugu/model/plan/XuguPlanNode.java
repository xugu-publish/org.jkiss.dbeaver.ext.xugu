...
//    }
    ...
    public Object getObject(DBRProgressMonitor monitor) throws DBException
    {
        ...
        if (XuguObjectType.INDEX.name().equals(objectTypeName)) {
            // Get index from parent table - reading of all indexes takes too much time
            for (XuguPlanNode parentNode = parent; parentNode != null; parentNode = parentNode.getParent()) {
                final Object parentObject = parentNode.getObject(monitor);
                if (parentObject instanceof XuguTablePhysical) {
                    return ((XuguTablePhysical) parentObject).getIndex(monitor, objectName);
                }
            }
            return objectName;
        } else {
            objectTypeName = XuguObjectType.TABLE.name();
        }
