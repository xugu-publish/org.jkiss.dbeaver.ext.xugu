...
package org.jkiss.dbeaver.ext.xugu.tools.sqldeveloper;
        ...
        try {
            ...
            for (Element refElement : XMLUtils.getChildElementList(configDocument.getDocumentElement(), "Reference")) {
                ...
                if (!CommonUtils.isEmpty(sid)) {
                    connectionInfo.setProviderProperty(XuguConstants.PROP_SID_SERVICE, XuguConnectionType.SID.name());
                } else if (!CommonUtils.isEmpty(serviceName)) {
                    connectionInfo.setProviderProperty(XuguConstants.PROP_SID_SERVICE, XuguConnectionType.SERVICE.name());
                }
                if (CommonUtils.toBoolean(osAuth)) {
                    connectionInfo.setUser(XuguConstants.OS_AUTH_PROP);
                }
                if (!CommonUtils.isEmpty(role)) {
                    connectionInfo.setProviderProperty(XuguConstants.PROP_INTERNAL_LOGON, role);
                }
