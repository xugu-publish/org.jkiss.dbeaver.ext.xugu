 ...
 * @author crowne
 */
public class JobRunHandler extends XuguTaskHandler
{
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException
    {
        ...
        final List<XuguSchedulerJob> objects = getSelectedJobs(event);
        if (!objects.isEmpty()) {
            ...
            if (objects.size() == 1) {
                final XuguSchedulerJob job = objects.get(0);
    ...
    private List<XuguSchedulerJob> getSelectedJobs(ExecutionEvent event)
    {
        List<XuguSchedulerJob> objects = new ArrayList<>();
        final ISelection currentSelection = HandlerUtil.getCurrentSelection(event);
        if (currentSelection instanceof IStructuredSelection && !currentSelection.isEmpty()) {
            for (Iterator<?> iter = ((IStructuredSelection) currentSelection).iterator(); iter.hasNext(); ) {
                final Object element = iter.next();
                final XuguSchedulerJob sourceJob = RuntimeUtils.getObjectAdapter(element, XuguSchedulerJob.class);
                if (sourceJob != null) {
        ...
        if (objects.isEmpty()) {
            ...
            final XuguSchedulerJob sourceJob = RuntimeUtils.getObjectAdapter(activePart, XuguSchedulerJob.class);
            if (sourceJob != null) {
                objects.add(sourceJob);
            }
        }
